...
from <- "c:/x/yft/mix2/Jitter_14c_m2_s10_a050_h80/Jitter_14c_m2_s10_a050_h80_10"
steep <- 0.65

steep.int <- round(100 * steep)
to <- gsub("_h80", paste0("_h", steep.int), from)
to <- basename(to)
mix <- as.integer(gsub(".*_m([0-9])_.*", "\\1", from))
species <- "yft"
memory <- "small"

top.dir <- paste0("../grid/h", steep.int)
